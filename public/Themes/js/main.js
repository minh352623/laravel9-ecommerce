    ...
    $(".js-show-modal1").on("click", function (e) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                // renderItem(response);
                items[0].dataset.thumb = response[0].feature_image_path;
                items[0].querySelector("img").src =
                    response[0].feature_image_path;
                items[0].querySelector("a").href =
                    response[0].feature_image_path;
                response[1].forEach((item, index) => {
                    if (index == items.length - 1) return;
                    items[index + 1].dataset.thumb = item.image_path;
                    items[index + 1].querySelector("img").src = item.image_path;
                    items[index + 1].querySelector("a").href = item.image_path;
                });
                name.textContent = response[0].name;
                price.textContent = "$" + response[0].price;
                description.innerHTML = response[0].content;
